...
# neural network (NN) with one hidden layer for binary
# classification for one hour ahead. The hour is specified in
# HOUR_AHEAD.
#
# The NN's output is either 0 if the input value presents rainfall
# amount of 0 mm or 1 in other cases.
#
...
# of occurrences of zeros and ones more equal.
# That it does not rain happens 89% of the time, thus for
# each sample resulting in rain, 7 new samples have been
# added with adding small random noise with Guassian
# distribution to existing measurements which result in rain.
#
...
#################     CONSTANTS     #################
HOUR_AHEAD = 12                                      # number of hour for which rainfall amounts are predicted
...
NUM_CLASS = 2                                       # number of labels/classes to classify the data into
...
STORE_DATA_PK_PATH = 'whole_model/hour_{hour}_model/bin_nn_data.pk'           # path to save .pk file with data for nn_data_in and nn_data_out
...
HOUR_MODEL_WEIGHTS_PATH = 'whole_model/hour_{hour}_model/bin_model'        # path where the weights of the completely trained model are saved - used later in rainfall prediction program

WRITE_MODEL_WEIGHTS = True                                                 # if True, writes the model weights to a CSV file specified in WRITE_WEIGHTS_PATH
WRITE_WEIGHTS_PATH = 'whole_model/hour_{hour}_model/bin_nn_weights.csv'    # path to writing weights of the model
...
checkpoint_path = "model_bin/training_save_files/cp-{epoch:04d}.ckpt"           # path for saving the temporary model weights during training - file name includes epoch number
...
if LOAD_DATA_PK==False:
    ...
    # Generate additional measurements for values with label 1 - for each 7 new
    ...
    for i in range(0,tmp_size):
        cur_val = nn_data_out[i][0]

        # if the current sample/row is 1
        if cur_val > 0.5:
            # generate N new samples with Guassian distribution
            N = 7                                    # number of new samples to generate per samples, which satisfies above criteria
...
#            print("Adding new sample at i={}".format(i))    # print, which row produces new samples (to know how long the process will take)
            for j in range(0,N):
                ...
                #new_sample = ["{0:.4f}".format(k) for k in new_sample]
                #new_sample = [str(k) for k in new_sample]

                nn_data_in.append(new_sample)    # append the new sample to the existing inputs

                nn_data_out.append([[1.0]])       # append the corresponding output to the existing outputs
